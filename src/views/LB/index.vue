			...
			<ul class="content-grid" v-if="tList.length > 0">
				...
				>
					<h3 class="title theme-color text-align-center">
						{{ cardTitle(item.conditional, item.interest) }}
					</h3>
					...
					<div class="cell">
						<p>
							<img
								class="icon"
								src="@/assets/icons/icon-LB.png"
								alt="errorImage"
							/>
							<span>LB</span>
						</p>
						<span class="theme-color">{{
							DepositedValue(item.value, item.conditional)
						}}</span>
					</div>
					...
					<div class="cell only-pc">
						<p>
							<img
								class="icon"
								src="@/assets/icons/icon-PEACE.png"
								alt="errorImage"
							/>
							<span>PEACE</span>
						</p>
						<span class="theme-color">{{
							textFromWei(item._calculate)
						}}</span>
					</div>
					...
					/>
					<button
						class="theme-bg-color submit"
						@click="join(item.amount, item.id)"
					>
						{{ $t("转入") }}
					</button>
					<button
...
async function getTList() {
	...
	try {
		...
		const Conditionals = await LBPOOLContract.methods
...
async function getConditionalValue() {
	...
	try {
		...
		const res = await LBPOOLContract.methods
			.get_conditional_value(userAddress.value, AbiAddressLB)
			.call();
		const _Res = res.map((item) =>
			ArrayKeysToObject(item, conditionalValueInterface)
		);
...
async function calculate(index) {
	try {
		const { LBPOOLContract } = Contracts.value;
		const res = await LBPOOLContract.methods
			.calculate(AbiAddressLB, userAddress.value, index)
			.call({
...
async function getReceived(index) {
	try {
		const { LBPOOLContract } = Contracts.value;
		const res = await LBPOOLContract.methods
			.get_received(AbiAddressLB, userAddress.value, index)
			.call({
				from: userAddress.value,
			});
...
async function join(value, id) {
	...
	try {
		...
		const balanceOf = await LBContract.methods
		...
		if (Number(fWeiBalanceOf) <= 0) {
			...
			PlusElMessage({
				type: "error",
				message:
					"The amount you entered is greater than the wallet balance!",
			});
		} else {
